...
class GenUL(loader.Module):
    ...
    #        await sleep(10)
    async def tripcalccmd(self, m: Message): #x - dose #n - days
        ''' - [DrugCalc] Калькулятор для расчета дозировки триптаминов таких как волшебные грибы и LSD
        * Обязательные аргументы:
        <last_dose> - последняя доза г/мкг 
        <days> - дней с момента употребления
        Пример: .tripcalc 170 5
        '''
        ...
        if args:
            ...
            return await m.edit("<b>🛑 ОШИБКА «не указананы аргументы»:</b> <i> - [последняя доза г/мкг] и [дней с момента употребления]</i>")
  ...
  <b>📋 Вам потребуется доза <code>{2}</code> μg (мкг) </b>чтобы почувствовать те же эффекты, что и ваша последняя доза.
            
  <b>💬 Обратите внимание❗️</b>, что этот калькулятор работает только для триптаминов, таких как ЛСД и волшебные грибы, не используйте этот калькулятор для химических веществ, которые не являются триптаминами.
Для наилучшего опыта, между каждым путешествием должно пройти 2 недели, толерантность должна вернуться к норме через 14 дней.
<b>Удачного путешествия!</b>❤️
            '''.format(dose, days, new)
        )
